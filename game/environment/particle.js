class Particle {
  ...
  run() {
    if(this.type === "spark") {
      ...
      this.time--;
      if(this.time <= 0 || !onScreen(this.x, this.y)) this.dead = true;
    }
    if(this.type === "blood") {
      ...
      if(this.time <= 0 || this.s <= 1 || !onScreen(this.x, this.y)) this.dead = true;
    }
    if(this.type === 'explosion') {
      this.s += (this.fscale - this.s)/20;
      this.op += -this.op/20;
      if(this.op <= 0.1 || !onScreen(this.x, this.y)) this.dead = true;
    }
    ...
    if(this.type === "fade") {
      this.op -= 5;
      this.s++;
      if(this.op <= 1 || !onScreen(this.x, this.y)) this.dead = true;
    }
    if(this.type === 'mine'){
      this.time--;
      for(let u of bodies) {
